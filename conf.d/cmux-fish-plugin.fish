# cmux shell integration plugin for fish
# https://github.com/grahamannett/cmux.fish
#
# Ported from the bash/zsh integrations in cmux:
# https://github.com/manaflow-ai/cmux/tree/main/Resources/shell-integration
#
# Communicates with the cmux app over a Unix domain socket to report:
#   - Current working directory
#   - Git branch and dirty state
#   - GitHub PR badge (via `gh pr view`)
#   - Shell activity state (prompt/running)
#   - TTY name (for port detection)
#   - Listening port scan triggers
...
function _cmux_send --description "Send a payload to the cmux socket"
    ...
    if command -v ncat >/dev/null 2>&1
        ...
        # macOS/BSD nc can hang on unix sockets. Try -N first, fall back to -w 1.
...
# ---------------------------------------------------------------------------
# _cmux_git_head_signature -- read first line of HEAD file as a signature.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _cmux_report_tty_once -- send TTY name to app once per session.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _cmux_kill_process_tree -- recursively kill a process and all children.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _cmux_stop_pr_poll_loop -- kill background PR poll loop if running.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _cmux_start_pr_poll_loop -- start (or restart) background PR poll loop.
#
# Runs as a background `fish -c` process that re-sources this plugin file
# via $_CMUX_PLUGIN_FILE so _cmux_send/_cmux_report_pr_for_path are available.
# ---------------------------------------------------------------------------
function _cmux_start_pr_poll_loop --description "Start or restart background PR poll loop"
    ...
    set -l plugin_file $_CMUX_PLUGIN_FILE
    ...
    set -l escaped_plugin (string replace -a "'" "\\'" -- $plugin_file)
...
# ---------------------------------------------------------------------------
function _cmux_fish_preexec --description "cmux preexec hook" --on-event fish_preexec
    test -S $CMUX_SOCKET_PATH; or return 0
    test -n "$CMUX_TAB_ID"; or return 0
    test -n "$CMUX_PANEL_ID"; or return 0
...
# ---------------------------------------------------------------------------
# Prompt hook -- called before each prompt is drawn.
# ---------------------------------------------------------------------------
function _cmux_fish_prompt --description "cmux prompt hook" --on-event fish_prompt
    test -S $CMUX_SOCKET_PATH; or return 0
    test -n "$CMUX_TAB_ID"; or return 0
    test -n "$CMUX_PANEL_ID"; or return 0
    ...
    # Clean up stale git probe processes.
    ...
    # Git branch/dirty: throttled async probe.
    set -l should_git 0
    ...
    if test $should_git -eq 1
        ...
        if test $can_launch_git -eq 1
            ...
            set -l escaped_plugin (string replace -a "'" "\\'" -- $_CMUX_PLUGIN_FILE)
            set -l tab_id $CMUX_TAB_ID
            set -l panel_id $CMUX_PANEL_ID
            fish -c "
set -x CMUX_SOCKET_PATH '$escaped_socket'
...
" >/dev/null 2>&1 &
    ...
    # Pull request: restart poll loop when directory or HEAD changes.
    ...
    # Ports: lightweight kick every ~10s.
...
# ---------------------------------------------------------------------------
# Store the plugin file path so background subprocesses can re-source it.
# Must come after all function definitions.
# ---------------------------------------------------------------------------
set -g _CMUX_PLUGIN_FILE (status filename)

# ---------------------------------------------------------------------------
# Cleanup on shell exit.
# ---------------------------------------------------------------------------
function _cmux_fish_exit --description "cmux cleanup on shell exit" --on-event fish_exit
    if test -n "$_CMUX_GIT_JOB_PID"
        _cmux_kill_process_tree $_CMUX_GIT_JOB_PID KILL
    end
    _cmux_stop_pr_poll_loop
